...
import { MAP_ASSET_KEYS, textureKeyForSpritePath } from "./mapAssets";
...
export function renderWorldMap(
  ...
  const lotsPlaced = bundleToPlacedLots(bundle);
  const buildingsPlaced = bundleToPlacedBuildings(bundle);
  const allEntities = [
  ...
  for (const t of bundle.tiles) {
    ...
    scene.add
      .image(
        cx,
        cy,
        textureKeyForSpritePath(t.spritePath, MAP_ASSET_KEYS.road),
      )
      .setDisplaySize(tw, th)
  ...
  for (const obj of bundle.objects) {
    ...
    scene.add
      .image(
        cx,
        cy,
        textureKeyForSpritePath(obj.spritePath, MAP_ASSET_KEYS.road),
      )
  ...
  for (const lot of bundle.lots) {
    ...
    scene.add
      .image(
        cx,
        cy,
        textureKeyForSpritePath(lot.spritePath, MAP_ASSET_KEYS.lot),
      )
  ...
  for (const b of bundle.buildings) {
    ...
    scene.add
      .image(
        cx,
        cy,
        textureKeyForSpritePath(b.spritePath, MAP_ASSET_KEYS.building),
      )
