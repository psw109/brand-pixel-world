...
2. **로컬 Supabase + Next 한 번에**(Docker 필요): `chmod +x scripts/dev_start.sh` 후 `./scripts/dev_start.sh`
...
#### 원격만 “다른 버전”이 찍혀 있을 때 (로컬·레포는 `20250401140000_bpw_core.sql`만 쓸 때)

Supabase는 DB 안에 **어떤 마이그레이션 버전을 이미 적용했는지** 테이블로 기록한다. 예전에 다른 PC·다른 파일 이름으로 `db push`를 했거나, SQL만 대시보드에서 돌린 뒤 기록이 남으면, **레포에는 없는 타임스탬프**가 원격 기록에만 남을 수 있다. 그때 `db push`는 “원격에 있는 버전이 로컬 폴더에 없다”고 실패한다.

**정본은 항상 레포 `supabase/migrations/`** 이므로, 원격 기록만 정리한 뒤 다시 push 하면 된다.

1. 해당 프로젝트에 링크: `npx supabase link --project-ref <ref>`
2. CI/CLI 에러에 나온 **레포에 없는 버전**을 `reverted` 처리 (스테이징에서 본 예시):

```bash
chmod +x scripts/supabase-align-remote-migrations.sh
./scripts/supabase-align-remote-migrations.sh
```

또는 직접:

```bash
npx supabase migration repair --status reverted 20260401113548 20260401115629 --linked
npx supabase db push
```

에러에 나온 숫자가 다르면 `SUPABASE_REPAIR_REVERT_VERSIONS="버전1 버전2"` 로 스크립트 실행 전에 지정하거나, 위 `repair` 줄의 버전만 바꾼다. **Preview(스테이징)와 Production은 각각** 링크를 바꿔 같은 절차를 반복한다.

**로컬 Docker Supabase**가 꼬였으면 데이터를 날려도 된다면 `npx supabase db reset`으로 `migrations/`만 다시 적용하는 것이 가장 단순하다.
...
- `supabase db push` 실패
  ...
  - `Remote migration versions not found in local` 이면 **§5.3** “원격만 다른 버전” 절차(`migration repair` + `db push` 또는 `scripts/supabase-align-remote-migrations.sh`)
