...
2. CI/CLI 에러에 나온 **레포에 없는 버전**을 `reverted` 처리한 뒤 push (스테이징에서 본 예시 — 숫자는 에러 메시지 그대로):
...
에러에 나온 버전이 다르면 위 `repair` 줄의 숫자만 바꾼다. **Preview(스테이징)와 Production은 각각** `link`를 바꿔 같은 절차를 반복한다.
...
- `supabase db push` 실패
  ...
  - `Remote migration versions not found in local` 이면 **§5.3** “원격만 다른 버전” 절차(`migration repair` + `db push`)
